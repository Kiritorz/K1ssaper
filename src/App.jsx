      ...
      try {
        ...
        if (sourcePath) {
           ...
           try {
              ...
              newPaper.localPdfPath = destPath; 
    ...
    return new Promise((resolve) => {
      try {
        ...
        if (paper && paper.localPdfPath) {
            try {
                if (fs.existsSync(paper.localPdfPath)) {
                    fs.unlinkSync(paper.localPdfPath);
                }
            } catch (err) {
...
const AdvancedMarkdown = ({ content }) => {
  ...
  useEffect(() => {
    ...
    const mathBlocks = [];
    let protectedText = content.replace(/\$\$([\s\S]+?)\$\$/g, (match, equation) => {
      const id = mathBlocks.length;
      mathBlocks.push({ type: 'block', equation });
      return `MATHBLOCK${id}ENDMATHBLOCK`;
    });

    protectedText = protectedText.replace(/(\$((?:[^\\$]|\\.)+)\$|\\\(([\s\S]+?)\\\))/g, (match, full, dollarContent, parenContent) => {
      const id = mathBlocks.length;
...
export default function K1ssaper() {
  ...
  // Fixed default absolute path for Windows as requested
  const [localPath, setLocalPath] = useState(() => localStorage.getItem('kpapers_local_path') || 'D:\\K1ssaper\\Storage');
  ...
  useEffect(() => {
    if (!document.querySelector('meta[http-equiv="Content-Security-Policy"]')) {
        ...
        meta.content = "default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob: file: https://cdn.jsdelivr.net;";
  ...
  useEffect(() => {
    setLoading(true);
    if (isElectron()) {
      FileSystemDB.getAll(localPath).then(data => {
        setPapers(data);
        setLoading(false);
      });
    } else {
  ...
  const getPdfUrl = (paper) => {
      if (isElectron() && paper.localPdfPath) {
          return `file://${paper.localPdfPath}`;
      }
      return null;
  };
  ...
  const downloadPdf = (paper) => {
      if (isElectron() && paper.localPdfPath) {
          if (window.require) {
              const { shell } = window.require('electron');
              shell.showItemInFolder(paper.localPdfPath);
          }
      } 
  };
  ...
  return (
    <div className="flex h-screen bg-[#F5F5F7] text-zinc-900 font-sans selection:bg-blue-100 selection:text-blue-900 overflow-hidden relative">
      ...
      <div className="w-64 flex-shrink-0 flex flex-col pt-4 pb-4 px-4 border-r border-transparent relative draggable"> 
        {/* App Title Text Only */}
        <div className="flex items-center gap-3 px-3 mb-8 mt-4 non-draggable">
        ...
        {activeTab === 'library' && (
            <div className="flex-1 flex flex-col min-h-0 non-draggable">
                     ...
                     {['All', 'Read', 'Unread'].map(filterKey => {
                         ...
                         return (
                            <button
                                ...
                                className={`w-full text-left px-3 py-2.5 rounded-xl text-[13px] flex items-center justify-between group ${
                                isActive 
                ...
                <div className="flex-1 overflow-y-auto space-y-1 pr-1 custom-scrollbar border-t border-zinc-200/50 pt-2">
                  <div className="px-3 py-2 text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-1 opacity-80">{t('groupsHeader')}</div>
                ...
                <div className="flex-1 p-12 overflow-y-auto">
                  <div className="max-w-2xl">
                      ...
                      <div className="mb-10 animate-in slide-in-from-top-4 fade-in duration-300">
                          ...
                          <p className="mt-2 text-xs text-zinc-400 px-2">
                              {t('localDesc')}
                          </p>
                      </div>
                      ...
                      <div className="mb-10">
                          ...
                          <div className="bg-zinc-50 rounded-2xl border border-zinc-200 p-6 flex items-center gap-4">
                              ...
                              <div className="ml-auto px-3 py-1 bg-zinc-200 rounded-full text-xs font-bold text-zinc-600 uppercase tracking-wide">
                                  {t('offline')}
                              </div>
                          </div>
                      </div>
                        ...
                        <div className={`grid ${gridColumns} gap-6 pb-12`}>
                            ...
                            >
                                ...
                                <div className="mt-auto">
                                    ...
                                    <span className="absolute right-0 bottom-0 text-[10px] text-zinc-400 font-medium tabular-nums transition-opacity duration-200 group-hover:opacity-0">
      ...
      {selectedPaper && (
         <>
            ...
            `}>
              ...
              {/* Note Header */}
              <div className="h-16 px-6 flex items-center justify-between border-b border-zinc-100/50 flex-shrink-0 bg-white/50 z-10 draggable">
                 <div className="flex items-center gap-4 non-draggable">
                    ...
                    <div className="flex bg-zinc-100/80 rounded-lg p-1">
                        ...
                        {/* PDF View Toggle */}
                        {(isElectron() && selectedPaper.localPdfPath) || (!isElectron() && selectedPaper.pdfFile) ? (
